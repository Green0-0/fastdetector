...
The long-running GPU job scripts request a warning five minutes before their time limit, stop the Python child, and use Slurm's native requeue operation to restart the same array task with the same resources. `SLURM_RESTART_COUNT` caps repeated restarts.
