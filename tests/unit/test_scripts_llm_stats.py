...
def plan_for(column_names, config) -> dict:
    """Mirror the per-pass plan main builds inline, for every configured suffix."""
    existing = set(column_names)
    return {
        col: missing
        for col in config.columns_to_score
        if (missing := output_columns(col, config.col_suffixes, config) - existing)
    }


def all_stems() -> list[str]:
    """Every per-model output column stem."""
    return list(PER_MODEL_METRICS)
...
# --------------------------------------------------------------------------
# planning (the set difference main does inline)
# --------------------------------------------------------------------------
...
def test_plan_requests_every_metric_for_a_fresh_dataset():
    ...
    plan = plan_for(["original", "final_response"], make_config())
...
def test_plan_skips_columns_that_already_exist():
    ...
    plan = plan_for(existing, make_config(columns_to_score=["original"]))
    assert plan["original"] == {
        "original_topp_outlier_a",
        "original_topk_outlier_a",
        "original_fastdetectgpt_a",
...
def test_plan_omits_a_fully_computed_column():
    ...
    assert plan_for(complete, config) == {}
...
def test_plan_skips_an_existing_binoculars_column():
    ...
    plan = plan_for(["original", "original_binoculars"], config)
...
def test_plan_is_empty_when_no_metrics_are_enabled():
    ...
    assert plan_for(["original"], config) == {}
...
def test_the_passes_together_cover_every_planned_column():
    ...
    needed = (output_columns('original', config.col_suffixes, config) - set(["original"]))
...
def test_plan_and_compute_agree_on_column_names():
    ...
    assert plan_for(["original", *result], config) == {}
