...
def make_llm_metrics_processor(
    ...
    def process_llm_metrics(examples):
        ...
        if stat_config.binoculars_score and len(llm_checkpoints) >= 2:
            ...
            for col in (col_a, col_b):
                # Binoculars needs M2 (performer) actual token logprobs
                # for the numerator (PPL_performer). Both columns are
                # already fetched by make_logprobs_processor — we just
                # need to grab the s2 (M2) column as well.
                ...
                result[f"{col}_binoculars"] = binoculars_scores_approx(
                    token_lps_m1, lp1, lp2, token_lps_m2
                )
